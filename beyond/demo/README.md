...
Assertions cover React rendering and state changes, original-consumer Beyond updates, retained internal state, real network consumption of ESM/SystemJS React artifacts, no browser runtime errors or failing HTTP responses, and no horizontal overflow at 390px. Screenshot inspection complements the assertions; it is not the execution test.

## CSS contract and evidence

`fixtures/app.css` imports `palette.css`; `shared.css` is an independent public module stylesheet. The fork builds each separately, with maps and `styles.graph.json` retaining its input relationships. The CSS is addressable as `/cdn/<format>/app.css` and `/cdn/<format>/shared.css`.
...
The test compares the same `.probe` selector in app, shared and document scopes: blue `rgb(36, 91, 117)`, orange `rgb(182, 83, 36)`, green `rgb(40, 84, 52)`. Both formats must preserve all three colors. This proves actual registration, serving, adoption and isolation for these artifacts. It does not implement Widgets controllers, Sass/Tailwind, automatic CSS HMR, failure-preserving replacement or notification lifecycle.
