...
Artifacts are `beyond/.cache/express/express.cjs` and `express.mjs`; `report.json` records resolved input, version, compiler provenance and public names. `cjs.graph.json` and `esm.graph.json` retain file edges, separate installed package identities (including Express's transitive dependencies), external builtins and output metadata. The graph records compiled reachability, not an exhaustive authoring/declaration graph or Beyond package resolver result.
